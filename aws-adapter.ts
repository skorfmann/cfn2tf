...
import { Stack, CfnElement } from 'monocdk-experiment';
...
export class AwsTerraformAdapter extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    new TerraformHost(scope, id, this);
  }
}
...
class TerraformHost extends Construct {
  ...
  constructor(scope: Construct, id: string, private readonly host: AwsTerraformAdapter) {
    super(scope, id);
  }
  ...
  private newTerraformResource(scope: Construct, logicalId: string, resource: CloudFormationResource): TerraformResource {
  ...
  private resolvePseudo(ref: string) {
    switch (ref) {
      case 'AWS::Partition': {
        this.awsPartition = this.awsPartition ?? new DataAwsPartition(this, 'aws-partition');
        return this.awsPartition.partition;
      }

      case 'AWS::Region': {
        this.awsRegion = this.awsRegion ?? new DataAwsRegion(this, 'aws-region');
        return this.awsRegion.name;
      }

      case 'AWS::AccountId': {
        this.awsCallerIdentity = this.awsCallerIdentity ?? new DataAwsCallerIdentity(this, 'aws-caller-identity');
  ...
  private resolveIntrinsic(fn: string, params: any) {
    switch (fn) {
      case 'Fn::GetAtt': {
        return Lazy.stringValue({ produce: () => this.resolveAtt(params[0], params[1]) });
      }

      case 'Fn::Join': {
        const [ delim, strings ] = params;
        return (strings as string[]).map(x => this.processIntrinsics(x)).join(delim);
      }
